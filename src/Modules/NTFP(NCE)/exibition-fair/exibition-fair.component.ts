...
@Component({
  selector: 'app-exibition-fair',
  templateUrl: './exibition-fair.component.html',
  styleUrl: './exibition-fair.component.css'
})
export class ExibitionFairComponent implements OnInit {
  ...
  columns: GridColumn[] = [
    ...
    { field: 'id', header: 'ID' },
    { field: 'product_sold_details', header: 'Product Sold Details' },
    { field: 'district', header: 'District' },
    { field: 'sub_division', header: 'Sub Division' },
    { field: 'range', header: 'Range' },
    { field: 'beat', header: 'Beat' },
    { field: 'stall_id_name', header: 'Stall Name' },
    { field: 'name_of_exibition', header: 'Exhibition Name' },
    { field: 'exibition_type', header: 'Exhibition Type' },
    { field: 'total_expense', header: 'Total Expense' },
    { field: 'prod_cat', header: 'Product Category' },
    { field: 'prod_subcat', header: 'Product Subcategory' },
    { field: 'num_prod_sold', header: 'Products Sold' },
    { field: 'num_of_queries', header: 'No. of Queries' },
    { field: 'rev_generation', header: 'Revenue Generated' },
  ...
  // ================= LOAD DATA =================
  Loadexibitionfair() {
    this.coreservices.getNtfpExhibitionFair(this.userid).subscribe({
      next: (res: any) => {
        const parsedRes = typeof res === 'string' ? JSON.parse(res) : res;
        let rawData = parsedRes?.Data ? (typeof parsedRes.Data === 'string' ? JSON.parse(parsedRes.Data) : parsedRes.Data) : [];
        sessionStorage.setItem("NTFP Existion Fair", JSON.stringify(rawData))
        this.data = [...rawData.map((item: any, index: number) => ({
          ...item,
          sno: index + 1,
          Main_module: "NTFP NCE",
          Sub_module: "Exibition and Fair",
          child_Module: "Exibition and Fair",

          ntfpexibitionfair: true,
  ...
  openImage(id: number, imageName: string) {
    ...
    this.coreservices.getNtfpExhibitionFairImage(id, imageName, 'ntfp_nce_market_exibition_fair').subscribe({
      next: (res: any) => {
  ...
  handleStatusUpdate(payload: any) {
    const module = "ntfp_nce_market_exibition_fair";
      ...
      this.coreservices.commonUpdates(module, Payload).subscribe({
        next: (res: any) => {
          if (res) {
            ...
            this.Loadexibitionfair(); // Refresh grid
          } else {
