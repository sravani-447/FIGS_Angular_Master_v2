...
@Component({
  selector: 'app-ntfp-marketing-stalls',
  templateUrl: './ntfp-marketing-stalls.component.html',
  styleUrl: './ntfp-marketing-stalls.component.css'
})
export class NTFPMarketingStallsComponent implements OnInit {
  ...
  columns: GridColumn[] = [
    ...
    { field: 'ntfp_prod_type', header: 'NTFP Product Type' },
    { field: 'ntfp_sub_prod_type', header: 'NTFP Sub Product Type' },
    { field: 'from_date', header: 'From Date' },
    { field: 'to_date', header: 'To Date' },
    { field: 'mode_of_sale', header: 'Mode of Sale' },
    { field: 'name_of_outlet', header: 'Outlet Name' },
    { field: 'quantity_sold', header: 'Quantity Sold' },
    { field: 'quantity_unit', header: 'Quantity Unit' },
    { field: 'selling_price', header: 'Selling Price' },
    { field: 'total_sale_value', header: 'Total Sale Value' },
...

    {
      ...
      // ================= LOAD DATA =================
      LoadntfpMarketingStalls() {
    this.coreservices.getNtfpMarketingSales(this.userid).subscribe({
      next: (res: any) => {
        ...
        this.data = [...rawData.map((item: any, index: number) => ({
          ...
          Sub_module:"Collection and Processsing Center",
          child_Module:"Marketing and Sales",

          ntfpMarketingStalls:true,
          sno: index + 1,
    ...
    this.coreservices.getNtfpMarketingSalesImage(id, imageName, 'ntfp_marketing_sales').subscribe({
      next: (res: any) => {
  ...
  switch (event.action) {
    case 'view_ntfpmarketing':
      ...
      const module = "ntfp_marketing_sales";
