...
@Component({
  selector: 'app-ntfp-survey',
  templateUrl: './ntfp-survey.component.html',
  styleUrl: './ntfp-survey.component.css'
})
export class NTFPSurveyComponent implements OnInit {
 ...
 { field: 'bo_rej_comments', header: 'Bo Reject Comments' },
  ...
  
  {
    ...
    actions: [
      ...
      // ================= LOAD DATA =================
      LoadntfpSurvey() {
    this.coreservices.getNtfpResourceMap(this.userid).subscribe({
      next: (res: any) => {
        ...
        this.data = [...rawData.map((item: any, index: number) => ({
          ...
          Sub_module: "Resource Assessment",
          child_Module: "Survey",
    ...
    this.coreservices.getNtfpResourceMapImage(id, imageName, 'ntfp_resource_map').subscribe({
      next: (res: any) => {
  ...
  switch (event.action) {
    ...
    try {
      ...
      a.download = `ntfp_survey_${row.site_id || 'data'}.kml`;
      a.href = url;
      ...
      const module = "ntfp_resource_map";
   ...
   this.coreservices.commonUpdates(module,Payload).subscribe({
    next: (res: any) => {
      if (res) {
        ...
        this.LoadntfpSurvey(); // Refresh grid
      } else {
