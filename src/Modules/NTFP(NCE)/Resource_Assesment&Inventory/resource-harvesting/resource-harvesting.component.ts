...
@Component({
  selector: 'app-resource-harvesting',
  templateUrl: './resource-harvesting.component.html',
  styleUrl: './resource-harvesting.component.css'
})
export class ResourceHarvestingComponent implements OnInit {
  ...
  columns: GridColumn[] = [
    {
      ...
      actions: [
        {
          label: 'View',
          action: 'view_resouce_harvesting',
          tooltip: 'View',
          icon: ''
        }
      ]
    ...
    { field: 'harvest_by', header: 'Harvest By' },
    { field: 'no_of_collectors', header: 'No. of Collectors' },
    ...
    { field: 'quality_of_resource', header: 'Quality' },
    { field: 'quantity_of_resource', header: 'Quantity' },
    { field: 'unit_of_quantity', header: 'Unit' },
    { field: 'harvesting_rate', header: 'Harvesting Rate' },
  ...
  // ================= LOAD DATA =================
  LoadresourceHarvesting() {
    this.coreservices.getNtfpResourceHarvesting(this.userid).subscribe({
      next: (res: any) => {
        ...
        this.data = [...rawData.map((item: any, index: number) => ({
          ...item,
          ntfpresourceHarvesting: true,
          sno: index + 1,
          Main_module: "NTFP NCE",
          Sub_module: " NTFP Resource Assesment",
          child_Module: "Resource Harvesting",
  ...
  openImage(id: number, imageName: string) {
    ...
    this.coreservices.getNtfpResourceHarvestingImage(id, imageName, 'ntfp_resource_harvesting').subscribe({
      next: (res: any) => {
  ...
  handleStatusUpdate(payload: any) {
    const module = "ntfp_resource_harvesting";
      ...
      this.coreservices.commonUpdates(module, Payload).subscribe({
        next: (res: any) => {
          if (res) {
            ...
            this.LoadresourceHarvesting(); // Refresh grid
          } else {
