...
@Component({
  selector: 'app-resource-generation-growth',
  templateUrl: './resource-generation-growth.component.html',
  styleUrl: './resource-generation-growth.component.css'
})
export class ResourceGenerationGrowthComponent implements OnInit {
  ...
  columns: GridColumn[] = [
    {
      ...
      actions: [
        {
          label: 'View',
          action: 'view_resource_generation',
          tooltip: 'View',
          icon: ''
        }
      ]
    ...
    { field: 'resource_type', header: 'Resource Type' },
    { field: 'resource_name', header: 'Resource Name' },
    { field: 'no_culm_per_clump', header: 'Culm per Clump' },
    { field: 'no_clum_per_hect', header: 'Clump per Hectare' },
    { field: 'reneration_status', header: 'Regeneration Status' },
    { field: 'survival_percentage', header: 'Survival %' },
    { field: 'average_height', header: 'Average Height' },
  ...
  // ================= LOAD DATA =================
  LoadresourceRegenerate() {
    this.coreservices.getNtfpResourceRegenerate(this.userid).subscribe({
      next: (res: any) => {
        ...
        this.data = [...rawData.map((item: any, index: number) => ({
          ...
          Sub_module: " NTFP Resource Assessment ",
          child_Module: "Resource Generation",
          ntfpresourceRegenerate: true,
          sno: index + 1,
  ...
  openImage(id: number, imageName: string) {
    ...
    this.coreservices.getNtfpResourceRegenerateImage(id, imageName, 'ntfp_resource_regenerate').subscribe({
      next: (res: any) => {
  ...
  handleStatusUpdate(payload: any) {
    const module = "ntfp_resource_regenerate";
      ...
      this.coreservices.commonUpdates(module, Payload).subscribe({
        next: (res: any) => {
          if (res) {
            ...
            this.LoadresourceRegenerate(); // Refresh grid
          } else {
