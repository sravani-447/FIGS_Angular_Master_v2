...
export class TargetDetailsComponent implements OnInit {
  ...
  columns: GridColumn[] = [
    ...
    { field: 'total_financial_target', header: 'Total Financial Target' },
  ...
  loadtargetdetails() {
    this.coreservices.getFiTargetDetails(this.userid).subscribe({
      next: (res: any) => {
  ...
  openImage(id: number, imageName: string) {
    ...
    this.coreservices.getFiTargetImage(id, imageName, 'fi_target_details').subscribe({
      next: (res: any) => {
  ...
  handleStatusUpdate(payload: any) {
    const module = "fi_target_details";
