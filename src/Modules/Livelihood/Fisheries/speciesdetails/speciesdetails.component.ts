...
@Component({
  ...
  styleUrl: './speciesdetails.component.css'
})
export class SpeciesdetailsComponent implements OnInit{
  ...
  { field: 'name_of_input', header: 'Name of Input' },
  { field: 'quantity', header: 'Quantity' },
  { field: 'total_culture_cost', header: 'Total Culture Cost' },
 ...
 { field: 'bo_rej_comments', header: 'Bo Reject Comments' },
  ...
  loadspeciesdetails() {
    this.coreservices.getFiSpeciesDetails(this.userid).subscribe({
      next: (res: any) => {
 ...
 sessionStorage.setItem("EcotourismCreation", JSON.stringify(rawData));
  ...
  // ================= IMAGE POPUP =================
    ...
    this.coreservices.getFiSpeciesImage(id, imageName, 'fi_species_details').subscribe({
      next: (res: any) => {
  ...
  switch (event.action) {
    case 'species_view':
      ...
      const module = "fi_species_details";
 ...
 if (Payload.status == "NO" && this.userDesignation=="RANGE_OFFICER") {
  ...
  if(Payload.Id !=null && Payload.comments != null){
   this.coreservices.commonUpdates('fi_species_details',Payload).subscribe({
    next: (res: any) => {
      if (res) {
        this.snackBar.open('Successfully updated', 'Close', { duration: 3000 });
        this.loadspeciesdetails(); // Refresh grid
      } else {
