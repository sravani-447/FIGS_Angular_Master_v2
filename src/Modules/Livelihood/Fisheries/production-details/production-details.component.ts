...
export class ProductionDetailsComponent implements OnInit{
  ...
  { field: 'self_cons_by_shg_jfmc', header: 'Self Consumption' },
  { field: 'sold_in_market', header: 'Sold in Market' },
  { field: 'total_production', header: 'Total Production' },
  { field: 'rate', header: 'Rate' },
  { field: 'total_income', header: 'Total Income' },
 ...
 { field: 'createdat', header: 'Creation Date' },
  ...
  loadproductiondetails() {
    this.coreservices.getFiProductionDetails(this.userid).subscribe({
      next: (res: any) => {
      ...
      this.data = [
        ...rawData.map((item: any, index: number) => ({
          ...
          child_Module: "Production Details",
  ...
  // ================= IMAGE POPUP =================
    ...
    this.coreservices.getFiProductionImage(id, imageName, 'fi_production_details').subscribe({
      next: (res: any) => {
  ...
  switch (event.action) {
    case 'production_view':
      ...
      const module = "fi_production_details";
   ...
   this.coreservices.commonUpdates(module,Payload).subscribe({
    next: (res: any) => {
      if (res) {
        ...
        this.loadproductiondetails(); // Refresh grid
      } else {
