...
@Component({
  selector: 'app-eco-tourism-siteselection',
  templateUrl: './eco-tourism-siteselection.component.html',
  styleUrl: './eco-tourism-siteselection.component.css'
})
export class EcoTourismSiteselectionComponent implements OnInit{
...
{ field: 'locality_name', header: 'Locality Name' },
  { field: 'selected_mode', header: 'Selection Mode' },
  { field: 'selected_id', header: 'Selected Value' },
  { field: 'site_x', header: 'Latitude' },
  { field: 'site_y', header: 'Longitude' },
 ...
 { field: 'bo_rej_comments', header: 'Bo Reject Comments' },
  ...
  loadsiteselection() {
    this.coreservices.getEcotourismSiteSelection(this.userid).subscribe({
       next: (res: any) => {
      ...
      sessionStorage.setItem("EcotourismSiteSelection", JSON.stringify(rawData));
      ...
      this.data = [
        ...rawData.map((item: any, index: number) => ({
          ...
          child_Module: "Eco Tourism Site Selection",
   ...
   this.coreservices.updateEcoTourismDetails(Payload).subscribe({
    next: (res: any) => {
      if (res && res.status === true) {
        ...
        this.loadsiteselection(); // Refresh grid
      } else {
