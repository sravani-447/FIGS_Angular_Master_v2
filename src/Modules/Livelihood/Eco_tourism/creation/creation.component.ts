...
export class CreationComponent implements OnInit{
  ...
  { field: 'selected_scheme', header: 'Selected Scheme' },
  { field: 'creation_type', header: 'Creation Type' },
  { field: 'name', header: 'Name' },
  { field: 'is_vicinity', header: 'Vicinity' },
  { field: 'distance_km', header: 'Distance (KM)' },
  { field: 'is_classroom', header: 'Classroom' },
  { field: 'is_admin_office', header: 'Admin Office' },
  { field: 'is_interpretation_center', header: 'Interpretation Center' },
 ...
 { field: 'bo_rej_comments', header: 'Bo Reject Comments' },
  ...
  loadcreation() {
    this.coreservices.getEcotourismCreation(this.userid).subscribe({
        next: (res: any) => {
  ...
  // ================= IMAGE POPUP =================
    ...
    this.coreservices.getEcoTourismCreationImage(id, imageName, 'eco_tourism_site_creation').subscribe({
      next: (res: any) => {
  ...
  switch (event.action) {
    case 'view':
      ...
      const module = "eco_tourism_site_creation";
 ...
 if (Payload.status == "NO" && this.userDesignation=="RANGE_OFFICER") {
  ...
  if(Payload.Id !=null && Payload.comments != null){
   this.coreservices.updateEcoCreationData(Payload).subscribe({
    next: (res: any) => {
      if (res && res.status === true) {
        this.snackBar.open('Successfully updated', 'Close', { duration: 3000 });
        this.loadcreation(); // Refresh grid
      } else {
