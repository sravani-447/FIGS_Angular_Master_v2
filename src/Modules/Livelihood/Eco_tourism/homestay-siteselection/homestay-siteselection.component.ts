...
@Component({
  selector: 'app-homestay-siteselection',
  templateUrl: './homestay-siteselection.component.html',
  styleUrl: './homestay-siteselection.component.css'
})
export class HomestaySiteselectionComponent implements OnInit{
    ...
    ngOnInit(): void {
      this.loadhomestaysiteselection();
  ...
  { field: 'ro_status', header: 'Status(RO)' },
   { field: 'homestay_id', header: 'Homestay ID' },
  { field: 'prox_to_tourism_site', header: 'Proximity to Tourism Site' },
  { field: 'health_sanitation', header: 'Health & Sanitation' },
  { field: 'power', header: 'Power' },
  { field: 'road_connectivity', header: 'Road Connectivity' },
 ...
 { field: 'bo_rej_comments', header: 'Bo Reject Comments' },
  ...
  loadhomestaysiteselection() {
    this.coreservices.getEcotourismHomestaySite(this.userid).subscribe({
       next: (res: any) => {
      ...
      sessionStorage.setItem("EcotourismHomestaySiteSelection", JSON.stringify(rawData));
      ...
      this.data = [
        ...rawData.map((item: any, index: number) => ({
          ...
          Sub_module: "Eco Tourism",
          child_Module: "Homestay Site Selection",
  ...
  // ================= IMAGE POPUP =================
    ...
    this.coreservices.getEcoTourismHomestayImage(id, imageName, 'eco_tourism_home_site_selection').subscribe({
      next: (res: any) => {
  ...
  switch (event.action) {
    case 'view':
      ...
      const module = "eco_tourism_home_site_selection";

  let Payload = {  
    "Id": payload.id,
 "prox_to_tourism_site": payload.prox_to_tourism_site,
 ...
 if (Payload.status == "NO" && this.userDesignation=="RANGE_OFFICER") {
  ...
  if(Payload.Id !=null && Payload.comments != null){
   this.coreservices.updateEcoHomestayData(Payload).subscribe({
    next: (res: any) => {
      if (res && res.status === true) {
        this.snackBar.open('Successfully updated', 'Close', { duration: 3000 });
        this.loadhomestaysiteselection(); // Refresh grid
      } else {
