...
@Component({
  selector: 'app-livelihood-monitoring',
  templateUrl: './livelihood-monitoring.component.html',
  styleUrl: './livelihood-monitoring.component.css'
})
export class LivelihoodMonitoringComponent implements OnInit{
  ...
  { field: 'monitor_type', header: 'Monitor Type' },
  { field: 'from_date', header: 'From Date' },
  { field: 'to_date', header: 'To Date' },
  { field: 'type_of_tourist', header: 'Tourist Type' },
  { field: 'revenue', header: 'Revenue' },
  { field: 'occupancy', header: 'Occupancy' },
  { field: 'no_of_tourist', header: 'No. of Tourists' },
  { field: 'carbon_footprint_mt', header: 'Carbon Footprint (MT)' },
 ...
 { field: 'bo_rej_comments', header: 'Bo Reject Comments' },
  ...
  loadlivelihoodmonitoring() {
    this.coreservices.getEcotourismMonitoring(this.userid).subscribe({
       next: (res: any) => {
  ...
  // ================= IMAGE POPUP =================
    ...
    this.coreservices.getEcoTourismMonitoringImage(id, imageName, 'eco_tourism_site_monitoring').subscribe({
      next: (res: any) => {
  ...
  switch (event.action) {
    case 'view':
      ...
      const module = "eco_tourism_site_monitoring";

  let Payload = {  
    "Id": payload.id,
      "no_of_tourist": payload.no_of_tourist,
 ...
 if (Payload.status == "NO" && this.userDesignation=="RANGE_OFFICER") {
  ...
  if(Payload.Id !=null && Payload.comments != null){
   this.coreservices.updateMonitorData(Payload).subscribe({
    next: (res: any) => {
      if (res && res.status === true) {
        this.snackBar.open('Successfully updated', 'Close', { duration: 3000 });
        this.loadlivelihoodmonitoring(); // Refresh grid
      } else {
