...
export class FacilitiesComponent implements OnInit{
  ...
  { field: 'homestay_type', header: 'Homestay Type' },
  { field: 'scheme_name', header: 'Scheme Name' },
  { field: 'homestay_rating', header: 'Rating' },
  { field: 'homestay_quality', header: 'Quality' },
  { field: 'homestay_quntity', header: 'Quantity' },
 ...
 { field: 'bo_rej_comments', header: 'Bo Reject Comments' },
  ...
  loadfacilities() {
    this.coreservices.getEcotourismFacilities(this.userid).subscribe({
        next: (res: any) => {
      ...
      sessionStorage.setItem("EcotourismFacilities", JSON.stringify(rawData));
  ...
  // ================= IMAGE POPUP =================
    ...
    this.coreservices.getEcoTourismFacilityImage(id, imageName, 'eco_tourism_facility').subscribe({
      next: (res: any) => {
  ...
  switch (event.action) {
    case 'view':
      ...
      const module = "eco_tourism_facility";

  let Payload = {  
    "Id": payload.id,
       "homestay_quntity": payload.homestay_quntity,
 ...
 if (Payload.status == "NO" && this.userDesignation=="RANGE_OFFICER") {
  ...
  if(Payload.Id !=null && Payload.comments != null){
   this.coreservices.updateFacilityData(Payload).subscribe({
    next: (res: any) => {
      if (res && res.status === true) {
        this.snackBar.open('Successfully updated', 'Close', { duration: 3000 });
        this.loadfacilities(); // Refresh grid
      } else {
