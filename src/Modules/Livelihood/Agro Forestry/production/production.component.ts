...
export class ProductionComponent implements OnInit{
  ...
  { field: 'date_of_harvest', header: 'Date of Harvest' },
  { field: 'qty_product_sold', header: 'Quantity Sold' },
  { field: 'production_unit', header: 'Production Unit' },
  { field: 'sold_per_rate', header: 'Sold Per Rate' },
  { field: 'incom_generate_sell_pro', header: 'Income Generated' },
  { field: 'self_consumed', header: 'Self Consumed' },
  { field: 'quantity_self_consumed', header: 'Quantity Self Consumed' },
  { field: 'gi_product', header: 'GI Product' },
 ...
 { field: 'bo_rej_comments', header: 'Bo Reject Comments' },
  ...
  loadproduction() {
    this.coreservices.getLivestockAfProduction(this.userid).subscribe({
      next: (res: any) => {
  ...
  // ================= IMAGE POPUP =================
    ...
    this.coreservices.getLivestockAfProductionImage(id, imageName, 'livestock_agro_production').subscribe({
      next: (res: any) => {
  ...
  switch (event.action) {
    case 'view':
      ...
      const module = "livestock_agro_production";

  let Payload = {  
    "Id": payload.id,
      Quantityofproductsold:  payload.qty_product_sold,
            Soldrateper:  payload.sold_per_rate,
 ...
 if (Payload.status == "NO" && this.userDesignation=="RANGE_OFFICER") {
  ...
  if(Payload.Id !=null && Payload.comments != null){
   this.coreservices.updateProductionDetails(Payload).subscribe({
    next: (res: any) => {
      if (res && res.status === true) {
        this.snackBar.open('Successfully updated', 'Close', { duration: 3000 });
        this.loadproduction(); // Refresh grid
      } else {
