...
@Component({
  selector: 'app-plantation-creation',
  templateUrl: './plantation-creation.component.html',
  styleUrl: './plantation-creation.component.css'
})
export class PlantationCreationComponent implements OnInit{
  ...
  { field: 'af_model', header: 'AF Model' },
  { field: 'space_bet_plant', header: 'Spacing Between Plants' },
  { field: 'date_of_planting', header: 'Date of Planting' },
  { field: 'no_seedling_proc', header: 'No. of Seedlings Procured' },
  { field: 'cost_of_seedling', header: 'Cost of Seedling' },
  { field: 'is_fencing_done', header: 'Fencing Done' },
  { field: 'length_of_fencing', header: 'Length of Fencing' },
  { field: 'cost_per_run_meter', header: 'Cost per Run Meter' },
  { field: 'mat_cost', header: 'Material Cost' },
  { field: 'mandays_male', header: 'Mandays (Male)' },
 ...
 { field: 'bo_rej_comments', header: 'Bo Reject Comments' },
  ...
  loadplantationcreation() {
    this.coreservices.getLivestockPlantCreation(this.userid).subscribe({
      next: (res: any) => {
      ...
      this.data = [
        ...rawData.map((item: any, index: number) => ({
          ...
          child_Module: "Plantation Creation",
  ...
  // ================= IMAGE POPUP =================
    ...
    this.coreservices.getLivestockAgroPlantationImage(id, imageName, 'livestock_agro_plantation').subscribe({
      next: (res: any) => {
  ...
  switch (event.action) {
    case 'view':
      ...
      const module = "livestock_agro_plantation";

  let Payload = {  
    Id: payload.id,
       Spacingbetweentheplants:  payload.space_bet_plant,
            Noofseedlingprocured:  payload.no_seedling_proc,
            Costofseedling: payload.cost_of_seedling,
            Lengthoffencing: payload.length_of_fencing,
            Costperrunningmeter: payload.cost_per_run_meter,
            NoofMandaysmale: payload.mandays_male,
            Noofmandaysfemale: payload.mandays_female,
 ...
 if (Payload.status == "NO" && this.userDesignation=="RANGE_OFFICER") {
  ...
  if(Payload.Id !=null && Payload.comments != null){
   this.coreservices.updateAgroPlantationDetails(Payload).subscribe({
    next: (res: any) => {
      if (res && res.status === true) {
        this.snackBar.open('Successfully updated', 'Close', { duration: 3000 });
        this.loadplantationcreation(); // Refresh grid
      } else {
