...
@Component({
  selector: 'app-resurvey',
  templateUrl: './resurvey.component.html',
  styleUrl: './resurvey.component.css'
})
export class ResurveyComponent implements OnInit{
  ...
  { field: 'ro_status', header: 'Status(RO)' },
   { field: 'area_hect', header: 'Area (Hectare)' },
 ...
 { field: 'bo_rej_comments', header: 'Bo Reject Comments' },
    ...
    actions: [
      {
        ...
        visible: (row: any) => !!row.site_id || !!row.lat 
      }
    ]
  }
];
  ...
  loadresurvey() {
    this.coreservices.getLivestockAfResurvey(this.userid).subscribe({
      next: (res: any) => {
  ...
  // ================= IMAGE POPUP =================
    ...
    this.coreservices.getLivestockAfResurveyImage(id, imageName, 'livestock_agro_resurvey').subscribe({
      next: (res: any) => {
  ...
  switch (event.action) {
    ...
    // Create GeoJSON dynamically from row
    const singleFeature = {
    ...
    try {
      ...
      a.download = `resurvey_site_${row.site_id || 'data'}.kml`;
      a.href = url;
      ...
      const module = "livestock_agro_resurvey";
 ...
 if (Payload.status == "NO" && this.userDesignation=="RANGE_OFFICER") {
  ...
  if(Payload.Id !=null && Payload.comments != null){
   this.coreservices.updateReSurveyDetails(Payload).subscribe({
    next: (res: any) => {
      if (res && res.status === true) {
        this.snackBar.open('Successfully updated', 'Close', { duration: 3000 });
        this.loadresurvey(); // Refresh grid
      } else {
