...
export class MaintenanceComponent implements OnInit{
  ...
  { field: 'maintenence_year', header: 'Maintenance Year' },
  { field: 'qty_hect', header: 'Quantity (Hectare)' },
  { field: 'mandays_male', header: 'Mandays (Male)' },
  { field: 'mandays_female', header: 'Mandays (Female)' },
  { field: 'unit_price', header: 'Unit Price' },
  { field: 'maintenance_cost', header: 'Maintenance Cost' },
 ...
 { field: 'bo_rej_comments', header: 'Bo Reject Comments' },
  ...
  loadmaintenance() {
    this.coreservices.getLivestockAfMaintenence(this.userid).subscribe({
      next: (res: any) => {
  ...
  // ================= IMAGE POPUP =================
    ...
    this.coreservices.getLivestockAfMaintenenceImage(id, imageName, 'livestock_agro_maintenence').subscribe({
      next: (res: any) => {
  ...
  switch (event.action) {
    case 'view':
      ...
      const module = "livestock_agro_maintenence";

  let Payload = {  
    "Id": payload.id,
       Quantityinha: payload.qty_hect,
            MandaysMale:payload.mandays_male,
 ...
 if (Payload.status == "NO" && this.userDesignation=="RANGE_OFFICER") {
  ...
  if(Payload.Id !=null && Payload.comments != null){
   this.coreservices.updateMaintanaceDetails(Payload).subscribe({
    next: (res: any) => {
      if (res && res.status === true) {
        this.snackBar.open('Successfully updated', 'Close', { duration: 3000 });
        this.loadmaintenance(); // Refresh grid
      } else {
