...
export class FieldDetailsComponent implements OnInit{
  ...
  { field: 'af_top_canopy', header: 'Top Canopy' },
  { field: 'af_middle_canopy', header: 'Middle Canopy' },
  { field: 'af_lower_canopy', header: 'Lower Canopy' },
  { field: 'af_creation_date', header: 'Creation Date' },
  { field: 'af_total_fin_target', header: 'Financial Target' },
  { field: 'af_ben_name', header: 'Beneficiary Name' },
 ...
 { field: 'bo_rej_comments', header: 'Bo Reject Comments' },
  ...
  loadfielddetails() {
    this.coreservices.getLivestockAfFieldDetails(this.userid).subscribe({
      next: (res: any) => {
  ...
  // ================= IMAGE POPUP =================
    ...
    this.coreservices.getLivestockAfFieldImage(id, imageName, 'livestock_agro_field_details').subscribe({
      next: (res: any) => {
  ...
  switch (event.action) {
    case 'view':
      ...
      const module = "livestock_agro_field_details";

  let Payload = {  
    Id: payload.id,
       TotalFinancialTarget:  payload.af_total_fin_target,
            BeneficiaryName: payload.af_ben_name,
 ...
 if (Payload.status == "NO" && this.userDesignation=="RANGE_OFFICER") {
  ...
  if(Payload.Id !=null && Payload.comments != null){
   this.coreservices.updateAgroFieldDetails(Payload).subscribe({
    next: (res: any) => {
      if (res && res.status === true) {
        this.snackBar.open('Successfully updated', 'Close', { duration: 3000 });
        this.loadfielddetails(); // Refresh grid
      } else {
