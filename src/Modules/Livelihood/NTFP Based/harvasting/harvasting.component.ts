...
@Component({
  selector: 'app-harvasting',
  templateUrl: './harvasting.component.html',
  styleUrl: './harvasting.component.css'
})
export class HarvastingComponent implements OnInit{
  ...
  { field: 'consume_type', header: 'Consume Type' },
  { field: 'expense_incurred', header: 'Expense Incurred' },
 ...
 { field: 'bo_rej_comments', header: 'Bo Reject Comments' },
  ...
  loadHarvasting() {
    this.coreservices.getNtfpBasedHarvesting(this.userid).subscribe({
      next: (res: any) => {
  ...
  // ================= IMAGE POPUP =================
    ...
    this.coreservices.getNtfpHarvestingImage(id, imageName, 'ntfp_based_harvasting').subscribe({
      next: (res: any) => {
  ...
  switch (event.action) {
    case 'view':
      ...
      const module = "ntfp_based_harvasting";
