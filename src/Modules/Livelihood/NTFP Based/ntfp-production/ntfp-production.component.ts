...
@Component({
  selector: 'app-ntfp-production',
  templateUrl: './ntfp-production.component.html',
  styleUrl: './ntfp-production.component.css'
})
export class NtfpProductionComponent implements OnInit{
  ...
  { field: 'shg_jlg_name', header: 'SHG & JLG Name' },
  { field: 'type_of_activity', header: 'Type of Activity' },
  { field: 'species_name', header: 'Species Name' },
  { field: 'rate_quintal', header: 'Rate (Quintal)' },
  { field: 'quantity_quintal', header: 'Quantity (Quintal)' },
  { field: 'total_cost', header: 'Total Cost' },
  { field: 'output_product', header: 'Output Product' },
  { field: 'rate_per_kg_no', header: 'Rate per Kg/No' },
  { field: 'quantity_kg_no', header: 'Quantity (Kg/No)' },
  ...
  loadNTFPProduction() {
    this.coreservices.getNtfpBasedProduction(this.userid).subscribe({
      next: (res: any) => {
      ...
      this.data = [
        ...rawData.map((item: any, index: number) => ({
          ...
          Sub_module: "NTFP Based",
          child_Module: "Production",
  ...
  // ================= IMAGE POPUP =================
    ...
    this.coreservices.getNtfpProductionImage(id, imageName, 'ntfp_based_production').subscribe({
      next: (res: any) => {
  ...
  switch (event.action) {
    case 'view':
      ...
      const module = "ntfp_based_production";
   ...
   this.coreservices.commonUpdates(module,Payload).subscribe({
    next: (res: any) => {
      if (res) {
        ...
        this.loadNTFPProduction(); // Refresh grid
      } else {
