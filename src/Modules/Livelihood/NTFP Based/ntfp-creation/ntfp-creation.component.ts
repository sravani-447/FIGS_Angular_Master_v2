...
@Component({
  selector: 'app-ntfp-creation',
  templateUrl: './ntfp-creation.component.html',
  styleUrl: './ntfp-creation.component.css'
})
export class NtfpCreationComponent implements OnInit{
  ...
  { field: 'secies_name', header: 'Species Name' },
  { field: 'ntfp_res_status', header: 'NTFP Resource Status' },
  { field: 'cost_incurred', header: 'Cost Incurred' },
  { field: 'type_of_material', header: 'Type of Material' },
  ...
  loadntfpcreation() {
    this.coreservices.getNtfpBasedCreation(this.userid).subscribe({
      next: (res: any) => {
  ...
  // ================= IMAGE POPUP =================
    ...
    this.coreservices.getNtfpImage(id, imageName, 'ntfp_based_creation').subscribe({
      next: (res: any) => {
  ...
  switch (event.action) {
    case 'view':
      ...
      const module = "ntfp_based_creation";
   ...
   this.coreservices.commonUpdates(module,Payload).subscribe({
    next: (res: any) => {
      if (res) {
        ...
        this.loadntfpcreation(); // Refresh grid
      } else {
