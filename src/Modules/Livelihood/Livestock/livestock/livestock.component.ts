...
@Component({
  ...
  styleUrl: './livestock.component.css'
})
export class LivestockComponent implements OnInit{
  ...
  { field: 'exist_facility', header: 'Existing Facility' },
  { field: 'new_facility', header: 'New Facility' },
  { field: 'new_facility_cost', header: 'New Facility Cost' },
  { field: 'ls_adult', header: 'Livestock Adult' },
  { field: 'ls_others', header: 'Livestock Others' },
  { field: 'commets', header: 'Comments' },
 ...
 { field: 'bo_rej_comments', header: 'Bo Reject Comments' },
  ...
  loadlivestock() {
    this.coreservices.getLiveStockMaster(this.userid).subscribe({
      next: (res: any) => {
 ...
 sessionStorage.setItem("EcotourismCreation", JSON.stringify(rawData));
  ...
  // ================= IMAGE POPUP =================
    ...
    this.coreservices.getLiveStockImage(id, imageName, 'livestock_master').subscribe({
      next: (res: any) => {
  ...
  switch (event.action) {
   ...
   handleStatusUpdate(payload: any) {
    const module = "livestock_master";

    let Payload = {  
      Id: payload.id, 
      Numberoflivestockadult: payload.ls_adult,           
      Numberoflivestockothers: payload.ls_others,           
      FacilitiesavailableExistingCreatedName: payload.exist_facility, 
      FacilitiesavailableNewlycreatedname: payload.new_facility,     
      FacilityavailableNewlyCreatedcost: payload.new_facility_cost,  
      commets: payload.comments || payload.commets,        
    ...
    if (Payload.status == "NO" && this.userDesignation == "RANGE_OFFICER") {
      this.coreservices.Insertcommonrejectdetails("RO", payload.rejectreason, Payload.Id, module).subscribe({
        next: () => { this.loadlivestock(); this.snackBar.open('Data Saved Successfully!', 'Close', { duration: 3000 }); }
      });
    }
    ...
    if (Payload.Id != null && Payload.commets != null) {
      this.coreservices.updateLivestockDetails(Payload).subscribe({
        next: (res: any) => {
          if (res && res.status === true) {
            this.snackBar.open('Successfully updated', 'Close', { duration: 3000 });
            this.loadlivestock(); // Refresh grid
          } else {
            this.snackBar.open('Update failed on server side', 'Close', { duration: 3000 });
