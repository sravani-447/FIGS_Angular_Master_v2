...
@Component({
  ...
  styleUrl: './stock-position.component.css'
})
export class StockPositionComponent implements OnInit{
  ...
  { field: 'no_of_livestock_adult', header: 'Livestock Adult' },
  { field: 'no_of_livestock_other', header: 'Livestock Other' },
  { field: 'product_sold_adult', header: 'Product Sold Adult' },
  { field: 'product_sold_other', header: 'Product Sold Other' },
  { field: 'mortality_adult', header: 'Mortality Adult' },
  { field: 'mortality_other', header: 'Mortality Other' },
  { field: 'self_consumtion_adult', header: 'Self Consumption Adult' },
  { field: 'self_consumtion_other', header: 'Self Consumption Other' },
  { field: 'new_birth', header: 'New Birth' },
  { field: 'new_purchase_adult', header: 'New Purchase Adult' },
  { field: 'new_purchase_other', header: 'New Purchase Other' },
  { field: 'cost_purchase_adult', header: 'Cost Purchase Adult' },
  { field: 'cost_purchase_other', header: 'Cost Purchase Other' },
  { field: 'total_cost', header: 'Total Cost' },
  { field: 'stock_available_adult', header: 'Stock Available Adult' },
  { field: 'stock_available_other', header: 'Stock Available Other' },
 ...
 { field: 'bo_rej_comments', header: 'Bo Reject Comments' },
  ...
  loadstockposition() {
    this.coreservices.getLiveStockStockPosition(this.userid).subscribe({
      next: (res: any) => {
  ...
  // ================= IMAGE POPUP =================
    ...
    this.coreservices.getLiveStockStockPositionImage(id, imageName, 'livestock_position').subscribe({
      next: (res: any) => {
  ...
  switch (event.action) {
    case 'view':
      ...
      const module = "livestock_position";
 ...
 if (Payload.status == "NO" && this.userDesignation=="RANGE_OFFICER") {
  ...
  if(Payload.Id !=null && Payload.comments != null){
   this.coreservices.updateStockPositionDetails(Payload).subscribe({
    next: (res: any) => {
      if (res && res.status === true) {
        this.snackBar.open('Successfully updated', 'Close', { duration: 3000 });
        this.loadstockposition(); // Refresh grid
      } else {
