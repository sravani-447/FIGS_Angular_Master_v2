...
@Component({
  ...
  styleUrl: './livestock-monitoring.component.css'
})
export class LivestockMonitoringComponent implements OnInit{
  ...
  { field: 'activity_livestock', header: 'Activity Livestock' },
  { field: 'activity_name', header: 'Activity Name' },
  { field: 'duration_from', header: 'Duration From' },
  { field: 'duration_to', header: 'Duration To' },
  { field: 'no_of_livestock', header: 'No. of Livestock' },
  { field: 'qty_helthy', header: 'Healthy Qty' },
  { field: 'qty_weak', header: 'Weak Qty' },
  { field: 'qty_adult', header: 'Adult Qty' },
  { field: 'qty_other', header: 'Other Qty' },
  { field: 'type_of_feed', header: 'Type of Feed' },
  { field: 'cost_feed', header: 'Feed Cost' },
  { field: 'is_vaccination', header: 'Vaccination' },
  { field: 'is_dewaring', header: 'Deworming' },
  { field: 'is_visit_vetnory_doc', header: 'Vet Visit' },
  { field: 'visit_date', header: 'Visit Date' },
  { field: 'is_required_if_visit_no', header: 'Required if No Visit' },
  { field: 'is_livestock_ins_covered', header: 'Insurance Covered' },
  { field: 'is_training_req_shg', header: 'Training Required' },
  { field: 'income_generated', header: 'Income Generated' },
 ...
 { field: 'bo_rej_comments', header: 'Bo Reject Comments' },
  ...
  loadlivestockmonitoring() {
    this.coreservices.getLiveStockMonitor(this.userid).subscribe({
      next: (res: any) => {
      ...
      this.data = [
        ...rawData.map((item: any, index: number) => ({
          ...
          Sub_module: "Livestock",
          child_Module: "Livestock Monitoring", 
  ...
  // ================= IMAGE POPUP =================
    ...
    this.coreservices.getLiveStockMonitorImage(id, imageName, 'livestock_monitor').subscribe({
      next: (res: any) => {
  ...
  switch (event.action) {
   ...
   handleStatusUpdate(payload: any) {
    const module = "livestock_monitor";

    let Payload = {  
      "Id": payload.id, 
      "Cost": payload.cost_feed,               
      "IncomegeneratedinRs": payload.income_generated, 
    ...
    if (Payload.status == "NO" && this.userDesignation == "RANGE_OFFICER") {
      this.coreservices.Insertcommonrejectdetails("RO", payload.rejectreason, Payload.Id, module).subscribe({
        next: () => { this.loadlivestockmonitoring(); this.snackBar.open('Data Saved Successfully!', 'Close', { duration: 3000 }); }
      });
    }
    ...
    if (Payload.Id != null && Payload.comments != null) {
      this.coreservices.updateLivestockMonitoringDetails(Payload).subscribe({
        next: (res: any) => {
          if (res && res.status === true) {
            this.snackBar.open('Successfully updated', 'Close', { duration: 3000 });
            this.loadlivestockmonitoring(); 
          } else {
            this.snackBar.open('Update failed on server', 'Close', { duration: 3000 });
            console.error("API returned false:", res);
          }
        },
        error: (err) => {
          console.error('API Error:', err);
          this.snackBar.open('Error contacting server', 'Close', { duration: 3000 });
        }
      });
    }
  }
