...
@Component({
  selector: 'app-catchmentmonitoring',
  templateUrl: './catchmentmonitoring.component.html',
  styleUrl: './catchmentmonitoring.component.css'
})
export class CatchmentmonitoringComponent implements OnInit {
  ...
  { field: 'mode', header: 'Model' },
  { field: 'type_of_structure', header: 'Type Of Structure' },
  { field: 'fishery_dome', header: 'Is Fishery Done' },
  ...
  { field: 'no_of_handpump', header: 'No Of Handpump' },
 ...
 { field: 'bo_rej_comments', header: 'Bo Reject Comments' },
  ...
  
  {
    ...
    // ================= LOAD DATA =================
  Loadcatchmentmonitoring() {
    this.coreservices.getAllCatchmentMonitoring(this.userid).subscribe({
      next: (res: any) => {
...
private getPopupHTML(row: any): string {
  ...
  setTimeout(() => {
    ...
    this.coreservices.gettMonitoringImage(id, imageName, 'catchment_smc_monitoring').subscribe({
      next: (res: any) => {
  ...
  handleStatusUpdate(payload: any) {
      const module = "catchment_smc_monitoring";

  let Payload = {  
    "Id": payload.id,
    "no_of_well": payload.no_of_well,
    "no_of_tube_well": payload.no_of_tube_well,
    "no_of_pond": payload.no_of_pond,
    "no_of_lake": payload.no_of_lake,
    "no_of_handpump": payload.no_of_handpump,
 ...
 if (Payload.status == "NO" && this.userDesignation=="RANGE_OFFICER") {
  ...
  if(Payload.Id !=null && Payload.comments != null){
   this.coreservices.commonUpdates("cm_monitoring_details",Payload).subscribe({
    next: (res: any) => {
      if (res) {
        this.snackBar.open('Successfully updated', 'Close', { duration: 3000 });
        this.Loadcatchmentmonitoring(); // Refresh grid
      } else {
