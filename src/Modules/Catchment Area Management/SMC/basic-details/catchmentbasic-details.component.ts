...
@Component({
  selector: 'app-catchmentbasic-details',
  templateUrl: './catchmentbasic-details.component.html',
  styleUrls: ['./catchmentbasic-details.component.css']
})
export class CatchmentBasicDetailsComponent implements OnInit {
 ...
 columns: GridColumn[] = [
  ...
  { field: 'id_mode', header: 'Selected Value' },
  { field: 'comments', header: 'Comments' },
  { field: 'user_name', header: 'Created By' },
...
loadCatchmentData() {
   this.coreservices.getAllCatchmentMaster(this.userid).subscribe({
      next: (res: any) => {
        const parsedRes = typeof res === 'string' ? JSON.parse(res) : res;
        let rawData = parsedRes?.Data ? JSON.parse(parsedRes.Data) : [];
        sessionStorage.setItem("CatchmenBasic",JSON.stringify(rawData))
...
handleStatusUpdate(payload: any) {
  const module = "catchment_smc_basicdetails";
  let Payload = {
    Id: payload.id,
    structure_id: payload.structure_id,
    type_of_structure: payload.type_of_structure,
  ...
  if(Payload.Id !=null && Payload.comments != null){
   this.coreservices.commonUpdates("cm_basic_details",Payload).subscribe({
    next: (res: any) => {
      if (res) {
        this.snackBar.open('Successfully updated', 'Close', { duration: 3000 });
        this.closeEditModal();
        this.loadCatchmentData(); // Refresh grid
      } else {
...
openImage(id: number, imageName: string) {
  ...
  this.popupImageUrl = 'assets/loading.gif'; // Or a local loading spinner

  this.coreservices.getCatchmentImage(id, imageName).subscribe({
    next: (res: any) => {
      const result = typeof res === 'string' ? JSON.parse(res) : res;
      if(result?.Data?.[0]?.img1_name!=null){
      const base64 = result?.Data?.[0]?.img;
  this.popupImageUrl = base64 ? `data:image/png;base64,${base64}` : 'assets/images/Image_not_available.png';
      }
      else{
        const base64 = result?.Data?.[0]?.img;
        this.popupImageUrl = base64 ? `data:image/png;base64,${base64}` : 'assets/images/Image_not_available.png';
    ...
    error: () => this.popupImageUrl = 'assets/images/Image_not_available.png'
  });
}
...
updateComments() {
  const payload = {
    ...
    type_of_structure: this.typeOfStructure,
    comments: this.editComments
  };

  this.coreservices.updateCatchmentComments(payload).subscribe({
    next: (res: any) => {
