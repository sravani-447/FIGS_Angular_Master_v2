...
@Component({
  selector: 'app-smc-site-suitability',
  templateUrl: './smc-site-suitability.component.html',
  styleUrl: './smc-site-suitability.component.css'
})
export class SmcSiteSuitabilityComponent implements OnInit{
 ...
 @ViewChild('grid') gridComponent!: CustomGridComponent;
  ...
  { field: 'pro_smc_structure', header: 'Proposed SMC Structure' },
  { field: 'soil_type', header: 'Soil Type' },
  { field: 'practical_feasibility', header: 'Practical feasibility' },
  { field: 'reason', header: 'Reason' },
  { field: 'ws_area', header: 'Watershed Area' },
  { field: 'water_stock_area', header: 'water stock area' },
 ...
 { field: 'bo_rej_comments', header: 'Bo Reject Comments' },
  ...
  
  {
    ...
    actions: [
     ...
     // ================= LOAD DATA =================
    Loadsitesuitability() {
    this.coreservices.getCatchmentSmcSiteSuitability(this.userid).subscribe({
      next: (res: any) => {
        ...
        this.data = [...rawData.map((item: any, index: number) => ({
          ...
          Sub_module: "Water Shed Management",
          child_Module: "SMC Site Suitability",

          catchmentsmcwatershed:true,
   ...
   private getPopupHTML(row: any): string {
    ...
    this.coreservices.getSiteSuitabilityImage(id, imageName, 'cm_smc_site_sutability').subscribe({
      next: (res: any) => {
  ...
  handleStatusUpdate(payload: any) {
      const module = "cm_smc_site_sutability";
 ...
 if (Payload.status == "NO" && this.userDesignation=="RANGE_OFFICER") {
  ...
  if(Payload.Id !=null && Payload.comments != null){
   this.coreservices.commonUpdates(module,Payload).subscribe({
    next: (res: any) => {
      if(res.message === "Successfuly Updated Records."){
      this.snackBar.open('Successfully updated', 'Close', { duration: 3000 });
        this.Loadsitesuitability(); // Refresh grid
      } else {
