...
export class WeatherDataComponent implements OnInit {
  ...
  { field: 'id', header: 'ID' },
  { field: 'date_taken', header: 'Date Taken' },
  { field: 'jfmc_name', header: 'JFMC' },
  { field: 'rainfall', header: 'Rainfall' },
  { field: 'rainfall_intensity', header: 'Rainfall Intensity' },
  { field: 'max_temp', header: 'Max Temp' },
  { field: 'min_temp', header: 'Min Temp' },
  { field: 'rainfall_humidity', header: 'Rainfall Humidity' },
  { field: 'wind_direction', header: 'Wind Direction' },
 ...
 { field: 'bo_rej_comments', header: 'Bo Reject Comments' },
  ...
  
  {
    ...
    actions: [
      ...
      // ================= LOAD DATA =================
      Loadweatherdata() {
    this.coreservices.getAllCatchmentWeatherData(this.userid).subscribe({
      next: (res: any) => {
        ...
        this.data = [...rawData.map((item: any, index: number) => ({
          ...
          child_Module: "Weather Data",

          catchmentsmcwaterweatherdata:true,
    ...
    this.coreservices.getWeatherImage(id, imageName, 'cm_weather_data').subscribe({
      next: (res: any) => {
   ...
   this.coreservices.commonUpdates(module,Payload).subscribe({
    next: (res: any) => {
      if (res) {
        ...
        this.Loadweatherdata(); // Refresh grid
      } else {
