...
@Component({
  ...
  templateUrl: './Live Immunization.component.html',
  styleUrls: ['./Live Immunization.component.css']
})
export class protectionliveimmunizationComponent implements OnInit {
...
const session = sessionStorage.getItem("Session");
  ...
  ngOnInit(): void {
    this.getprotectionLiveImunizationdetails();
...
columns: GridColumn[] = [
  ...
  { field: 'livestock_type', header: 'Livestock Type' },

  // Immunization Info
  { field: 'immunization_done', header: 'Immunization Done' },
  { field: 'date_of_immunization', header: 'Date Of Immunization' },
  { field: 'if_no_reason', header: 'If No Reason' },
...
getprotectionLiveImunizationdetails() {
  this.coreservices.getliveimmunizationdetailsdatalist(this.userid).subscribe({
     next: (res: any) => {
      ...
      this.data = [
        ...rawData.map((item: any, index: number) => ({
          ...
          child_Module: "Livestock immunization",
 ...
 openLocationOnMap(row: any) {
  ...
  openImage(id: string, imageName: string) {
    ...
    this.coreservices.getprotectionliveimmunizationImages(id, imageName).subscribe({
      next: (res: any) => {
  ...
  handleStatusUpdate(payload: any) {
    ...
    if (Payload.comments != undefined && Payload.status != "NO" && Payload.status != "YES") {
      this.coreservices.InsertProtectionliveimmunizatondetails(Payload).subscribe({
        next: (res: any) => {
          this.getprotectionLiveImunizationdetails();
      ...
      this.coreservices.Insertcommonrejectdetails("RO", Payload.rejectreason, Payload.Id,"ecod_prot_lives_immu").subscribe({
        next: (res: any) => {
          this.getprotectionLiveImunizationdetails();
          this.snackBar.open('Data Saved Successfully!', 'Close', { duration: 3000 });
        },
      ...
      this.coreservices.CommonApprovalforallroles("FO", "BO", Payload.Id, "ecod_prot_lives_immu").subscribe({
        next: (res: any) => {
          this.getprotectionLiveImunizationdetails();
          this.snackBar.open('Data Saved Successfully!', 'Close', { duration: 3000 });
        },
      ...
      this.coreservices.CommonApprovalforallroles("BO", "RO", Payload.Id, "ecod_prot_lives_immu").subscribe({
        next: (res: any) => {
          this.getprotectionLiveImunizationdetails();
          this.snackBar.open('Data Saved Successfully!', 'Close', { duration: 3000 });
        },
      ...
      this.coreservices.commonApprovedByRangeo(Payload.Id,"ecod_prot_lives_immu").subscribe({
        next: (res: any) => {
          this.getprotectionLiveImunizationdetails();
          this.snackBar.open('Data Saved Successfully!', 'Close', { duration: 3000 });
        },
