...
@Component({
  selector: 'app-creation.component.component',
  templateUrl: './creation.component.html',
  styleUrls: ['./creation.component.css']
})
export class protectionCreationComponent implements OnInit {
...
columns: GridColumn[] = [
  ...
  // Basic Info
  { field: 'id', header: 'Id' },
  { field: 'protection_id', header: 'Protection Id' },
  { field: 'protection_type', header: 'Protection Type' },
  { field: 'beat_name', header: 'Beat Name' },
  { field: 'date_of_creation', header: 'Date Of Creation' },
  ...
  // Work Details
  { field: 'length_of_runnning_meter', header: 'Length Of Running Meter' },
  { field: 'type_of_fencing', header: 'Type Of Fencing' },
  { field: 'pillar_type', header: 'Pillar Type' },
  { field: 'if_other', header: 'Other' },
...
getCreationdetails() {
  this.coreservices.getprotectioncreationdetailsdatalist(this.userid).subscribe({
     next: (res: any) => {
 ...
 openLocationOnMap(row: any) {
  ...
  openImage(id: string, imageName: string) {
    ...
    this.coreservices.getprotectioncreationImages(id, imageName).subscribe({
      next: (res: any) => {
  ...
  onGridAction(event: { action: string; row: any }) {
    ...
    let Payload = {
      ...
      Lengthinrunningmeter: payload.length_of_runnning_meter,
    ...
    if (Payload.comments != undefined && Payload.status != "NO" && Payload.status != "YES") {
      this.coreservices.InsertProtectioncreationdetails(Payload).subscribe({
        next: (res: any) => {
          this.getCreationdetails();
      ...
      this.coreservices.Insertcommonrejectdetails("RO", Payload.rejectreason, Payload.Id,"ecod_prot_create").subscribe({
        next: (res: any) => {
          this.getCreationdetails();
          this.snackBar.open('Data Saved Successfully!', 'Close', { duration: 3000 });
        },
      ...
      this.coreservices.CommonApprovalforallroles("FO", "BO", Payload.Id, "ecod_prot_create").subscribe({
        next: (res: any) => {
          this.getCreationdetails();
          this.snackBar.open('Data Saved Successfully!', 'Close', { duration: 3000 });
        },
      ...
      this.coreservices.CommonApprovalforallroles("BO", "RO", Payload.Id, "ecod_prot_create").subscribe({
        next: (res: any) => {
          this.getCreationdetails();
          this.snackBar.open('Data Saved Successfully!', 'Close', { duration: 3000 });
        },
      ...
      this.coreservices.commonApprovedByRangeo(Payload.Id,"ecod_prot_create").subscribe({
        next: (res: any) => {
          this.getCreationdetails();
          this.snackBar.open('Data Saved Successfully!', 'Close', { duration: 3000 });
        },
