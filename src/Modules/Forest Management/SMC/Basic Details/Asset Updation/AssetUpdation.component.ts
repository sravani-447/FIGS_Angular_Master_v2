...
@Component({
  selector: 'app-AssetUpdation.component.component',
  templateUrl: './AssetUpdation.component.html',
  styleUrls: ['./AssetUpdation.component.css']
})
export class smcAssetUpdationComponent implements OnInit {
...
getassetUpdationdetails() {
  this.coreservices.getsmcassetupdationdetailsdatalist(this.userid).subscribe({
     next: (res: any) => {
 ...
 openLocationOnMap(row: any) {
  ...
  openImage(id: string, imageName: string) {
    ...
    this.coreservices.getsmcassetupdationImages(id, imageName).subscribe({
      next: (res: any) => {
  ...
  onGridAction(event: { action: string; row: any }) {
   ...
   handleStatusUpdate(payload: any) {
    ...
    if (Payload.comments != undefined && Payload.status != "NO" && Payload.status != "YES") {
      this.coreservices.InsertForestSmcassetupdationdetails(Payload).subscribe({
        next: (res: any) => {
          this.getassetUpdationdetails();
      ...
      this.coreservices.Insertcommonrejectdetails("RO", Payload.rejectreason, Payload.Id,"ecod_smc_asset_updation").subscribe({
        next: (res: any) => {
          this.getassetUpdationdetails();
          this.snackBar.open('Data Saved Successfully!', 'Close', { duration: 3000 });
        },
      ...
      this.coreservices.CommonApprovalforallroles("FO", "BO", Payload.Id, "ecod_smc_asset_updation").subscribe({
        next: (res: any) => {
          this.getassetUpdationdetails();
          this.snackBar.open('Data Saved Successfully!', 'Close', { duration: 3000 });
        },
      ...
      this.coreservices.CommonApprovalforallroles("BO", "RO", Payload.Id, "ecod_smc_asset_updation").subscribe({
        next: (res: any) => {
          this.getassetUpdationdetails();
          this.snackBar.open('Data Saved Successfully!', 'Close', { duration: 3000 });
        },
      ...
      this.coreservices.commonApprovedByRangeo(Payload.Id,"ecod_smc_asset_updation").subscribe({
        next: (res: any) => {
          this.getassetUpdationdetails();
          this.snackBar.open('Data Saved Successfully!', 'Close', { duration: 3000 });
        },
