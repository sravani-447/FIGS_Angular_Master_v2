...
@Component({
  selector: 'app-basicdetails.component',
  templateUrl: './basicdetails.component.html',
  styleUrls: ['./basicdetails.component.css']
})
export class smcbasicdetailsComponent implements OnInit {
...
columns: GridColumn[] = [
  ...
  { field: 'structure_id', header: 'Structure_Id' },
  { field: 'type_of_scheme', header: 'Type_of_Scheme' },
  { field: 'type_of_structure', header: 'Type_of_Structure' },
  { field: 'sel_mode', header: 'Mode' },
  { field: 'id_mode', header: 'Mode Type Name' },
...
getsmcbasicdetails() {
  this.coreservices.getsmcbasicdetailsdatalist(this.userid).subscribe({
    next: (res: any) => {
 ...
 openLocationOnMap(row: any) {
  ...
  openImage(id: string, imageName: string) {
    ...
    this.coreservices.getsmcbasicdetailsImages(id, imageName).subscribe({
      next: (res: any) => {
  ...
  handleStatusUpdate(payload: any) {
    ...
    if (Payload.comments != undefined && Payload.status != "NO" && Payload.status != "YES") {
      this.coreservices.InsertForestSmcbasicupdationdetails(Payload).subscribe({
        next: (res: any) => {
          this.getsmcbasicdetails();
      ...
      this.coreservices.Insertcommonrejectdetails("RO", Payload.rejectreason, Payload.Id,"ecod_smc_basic").subscribe({
        next: (res: any) => {
          this.getsmcbasicdetails();
          this.snackBar.open('Data Saved Successfully!', 'Close', { duration: 3000 });
        },
      ...
      this.coreservices.CommonApprovalforallroles("FO", "BO", Payload.Id, "ecod_smc_basic").subscribe({
        next: (res: any) => {
          this.getsmcbasicdetails();
          this.snackBar.open('Data Saved Successfully!', 'Close', { duration: 3000 });
        },
      ...
      this.coreservices.CommonApprovalforallroles("BO", "RO", Payload.Id, "ecod_smc_basic").subscribe({
        next: (res: any) => {
          this.getsmcbasicdetails();
          this.snackBar.open('Data Saved Successfully!', 'Close', { duration: 3000 });
        },
      ...
      this.coreservices.commonApprovedByRangeo(Payload.Id,"ecod_smc_basic").subscribe({
        next: (res: any) => {
          this.getsmcbasicdetails();
          this.snackBar.open('Data Saved Successfully!', 'Close', { duration: 3000 });
        },
