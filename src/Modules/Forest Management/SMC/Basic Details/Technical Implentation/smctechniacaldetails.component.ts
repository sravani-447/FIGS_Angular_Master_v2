  ...
  actions: [
    {
      ...
      visible: (row: any) => row.estimate_pdf_name && row.estimate_pdf_name !== ""
    }
  ]
},
  { field: 'location', header: 'Location' },

  // Water Harvesting Dimensions
  { field: 'wh_length', header: 'WH Length' },
  { field: 'wh_breadth', header: 'WH Breadth' },
  { field: 'wh_height', header: 'WH Height' },
  { field: 'wh_volume', header: 'WH Volume' },

  // Labour Details
  { field: 'mandays_man', header: 'Mandays Man' },
  { field: 'mandays_female', header: 'Mandays Female' },
  { field: 'mandays_cost', header: 'Mandays Cost' },
...
getsmctechnicaldetails() {
     ...
     next: (res: any) => {
      ...
      this.data = [
        ...rawData.map((item: any, index: number) => ({
          ...
          Sub_module: "SMC",
          child_Module: "Technical Implementation",
 ...
 openLocationOnMap(row: any) {
  ...
  openImage(id: string, imageName: string) {
    ...
    this.coreservices.getsmcetechnicaldetailsImages(id, imageName).subscribe({
      next: (res: any) => {
  ...
  onGridAction(event: { action: string; row: any }) {
    ...
    switch (event.action) {
        case 'download_pdf':
        const url = this.coreservices.getTechnicalPdfUrl(row.id);
        
        const link = document.createElement('a');
        link.href = url;
        
        link.setAttribute('download', row.estimate_pdf_name || 'estimate.pdf');
 ...
 downloadPdf(id: string, fileName: string) {
    const url = this.coreservices.getTechnicalPdfUrl(id);
    ...
    let Payload = {
      ...
      Length:payload.wh_length,
      Breadth:payload.wh_breadth,
      Height:payload.wh_height,
      Volume:payload.wh_volume,
      MandaysMale:payload.mandays_man,
      MandaysFemale:payload.mandays_female,
      MandaysCost:payload.mandays_cost,
    ...
    if (Payload.comments != undefined && Payload.status != "NO" && Payload.status != "YES") {
      this.coreservices.InsertForestsmctechnicaldetails(Payload).subscribe({
        next: (res: any) => {
          this.getsmctechnicaldetails();
      ...
      this.coreservices.Insertcommonrejectdetails("RO", Payload.rejectreason, Payload.Id,"ecod_smc_technical").subscribe({
        next: (res: any) => {
          this.getsmctechnicaldetails();
          this.snackBar.open('Data Saved Successfully!', 'Close', { duration: 3000 });
        },
      ...
      this.coreservices.CommonApprovalforallroles("FO", "BO", Payload.Id, "ecod_smc_technical").subscribe({
        next: (res: any) => {
          this.getsmctechnicaldetails();
          this.snackBar.open('Data Saved Successfully!', 'Close', { duration: 3000 });
        },
      ...
      this.coreservices.CommonApprovalforallroles("BO", "RO", Payload.Id, "ecod_smc_technical").subscribe({
        next: (res: any) => {
          this.getsmctechnicaldetails();
          this.snackBar.open('Data Saved Successfully!', 'Close', { duration: 3000 });
        },
      ...
      this.coreservices.commonApprovedByRangeo(Payload.Id,"ecod_smc_technical").subscribe({
        next: (res: any) => {
          this.getsmctechnicaldetails();
          this.snackBar.open('Data Saved Successfully!', 'Close', { duration: 3000 });
        },
