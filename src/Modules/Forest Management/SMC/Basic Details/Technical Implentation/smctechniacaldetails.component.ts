...
@Component({
  ...
  templateUrl: './smctechniacaldetails.component.html',
  styleUrls: ['./smctechniacaldetails.component.css']
})
export class smctechniacaldetailsComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.getsmctechnicaldetails();
  }

  getsmctechnicaldetails() {
    this.coreservices.getsmcTechnicaldetailsdatalist(this.userid).subscribe({
      next: (res: any) => {
        let parsed = typeof res === 'string' ? JSON.parse(res) : res;
        let rawData = parsed?.Data ? JSON.parse(parsed.Data) : [];
  ...
  columns: GridColumn[] = [
    ...
    { field: 'type_of_structure', header: 'Type Of Structure' },
    { field: 'location', header: 'Location' },
    { field: 'wh_length', header: 'Length' },
    { field: 'wh_breadth', header: 'Breadth' },
    { field: 'wh_height', header: 'Height' },
    { field: 'wh_volume', header: 'Volume' },
    { field: 'mandays_man', header: 'Man Days (Male)' },
    { field: 'mandays_female', header: 'Man Days (Female)' },
    { field: 'mandays_cost', header: 'Man Days Cost' },
  ...
  isPdfUploaded(): boolean {
    ...
    return this.selectedRow.estimatefile_status === true || 
           ...
           !!this.selectedRow.estimate_pdf_name;
  }
  ...
  async uploadPdf() {
    ...
    try {
      ...
      const payload = {
        Modulename: 'y_ecodevelopment.eco_smc_technical_details', // Explicitly targeting Forest Management table profile
        filename: fileToUpload.name,
        File: byteArray,
        Filetype: 'smc_forest_estimatePdf',
      ...
      this.coreservices.commonuploadPdf(payload).subscribe({
        next: () => {
          ...
          this.getsmctechnicaldetails(); // Refresh grid layout context
        },
        error: (err: any) => {
          console.error(err);
  ...
  downloadCurrentPdf() {
    const fileName = this.selectedRow.estimate_pdf_name || 'Forest_Management_Technical_Details.pdf';
  ...
  // ================= MAP AND IMAGE HANDLING =================
  initMap(): void {
    if (this.map) this.map.remove();
    this.map = L.map('map', { zoomControl: false }).setView([17.3850, 78.4867], 6);
  ...
  openLocationOnMap(row: any) {
    ...
    const marker = L.marker([lat, lng]).bindPopup(`<b>Structure ID:</b> ${row.structure_id}`).openPopup();
  ...
  openImage(id: number, imageName: string) {
    ...
    this.coreservices.getTechnicalImage(id, imageName).subscribe({
      next: (res: any) => {
        ...
        this.popupImageUrl = base64 ? 'data:image/jpeg;base64,' + base64 : 'https://placehold.co/600x400?text=No+Image';
      },
      error: () => { this.popupImageUrl = 'https://placehold.co/600x400?text=Error'; }
    });
  }
  ...
  handleStatusUpdate(payload: any) {
    const module = "ecod_smc_technical";
    let Payload = {
      Id: payload.id,
      location: payload.location,
      comments: payload.comments,
      wh_length: payload.wh_length,
      wh_breadth: payload.wh_breadth,
      wh_height: payload.wh_height,
      wh_volume: payload.wh_volume,
      mandays_man: payload.mandays_man,
      mandays_female: payload.mandays_female,
      mandays_cost: payload.mandays_cost,
    ...
    if (Payload.status === "NO" && this.userDesignation === "RANGE_OFFICER") {
      this.coreservices.Insertcommonrejectdetails("RO", payload.rejectreason, Payload.Id, module).subscribe({
        next: () => { this.getsmctechnicaldetails(); this.snackBar.open('Rejected Successfully', 'Close', { duration: 3000 }); }
      });
    } 
      ...
      this.coreservices.CommonApprovalforallroles("FO", "BO", Payload.Id, module).subscribe({
        next: () => { this.getsmctechnicaldetails(); this.snackBar.open('Data Saved Successfully!', 'Close', { duration: 3000 }); }
      });
    }
    ...
    if (Payload.Id != null && Payload.comments != null) {
      this.coreservices.commonUpdates("ecod_smc_technical", Payload).subscribe({
        next: (res: any) => {
          if (res) {
