...
export class smcImplementationdetailsComponent implements OnInit {
  ...
  downloadMasterRolePdf(id: any, name: string) {
    this.selectedRow = this.data.find(item => item.id === id) || { id: id };
    this.DownloadPdf(this.selectedRow, name);
  }
  ...
  async uploadPdf() {
    ...
    try {
      ...
      const payload = {
        Modulename: 'y_ecodevelopment.eco_smc_implementation',
        File: byteArray,
        filename: fileToUpload.name,
        Filetype: 'smc_forestmasterRollPdf',
      ...
      this.snackBar.open('Failed to process file.', 'Close', { duration: 3000 });
    }
  }

  downloadCurrentPdf() {
    const fileName = this.selectedRow?.master_roll_pdf_name || 'Eco_SMC_Implementation_MasterRoll.pdf';
