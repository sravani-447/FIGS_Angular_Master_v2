...
@Component({
  selector: 'app-Montoring.component.component',
  templateUrl: './Montoring.component.html',
  styleUrls: ['./Montoring.component.css']
})
export class smcMontoringComponent implements OnInit {
...
columns: GridColumn[] = [
  ...
  // Structure Details
  { field: 'fishery_dome', header: 'Fishery Dome' },
  { field: 'shg_id', header: 'SHG & JLG' },

  // Water Source Counts
  { field: 'no_of_well', header: 'No of Well' },
  { field: 'no_of_tube_well', header: 'No of Tube Well' },
  { field: 'no_of_pond', header: 'No of Pond' },
  { field: 'no_of_lake', header: 'No of Lake' },
  { field: 'no_of_handpump', header: 'No of Hand Pump' },
...
getsmcMontoringdetails() {
  this.coreservices.getsmcMontoringdetailsdatalist(this.userid).subscribe({
     next: (res: any) => {
 ...
 openLocationOnMap(row: any) {
  ...
  openImage(id: string, imageName: string) {
    ...
    this.coreservices.getsmcmontoringImages(id, imageName).subscribe({
      next: (res: any) => {
  ...
  onGridAction(event: { action: string; row: any }) {
   ...
   handleStatusUpdate(payload: any) {
    ...
    let Payload = {
      ...
      Numberofwell: payload.no_of_well,
      Numberoftubewell:payload.no_of_tube_well,
      Numberofpond:payload.no_of_pond,
      Numberoflakes:payload.no_of_lake,
      Numberofhandpump:payload.no_of_handpump,
    ...
    if (Payload.comments != undefined && Payload.status != "NO" && Payload.status != "YES") {
      this.coreservices.InsertSmcMontoringdetails(Payload).subscribe({
        next: (res: any) => {
          this.getsmcMontoringdetails();
      ...
      this.coreservices.Insertcommonrejectdetails("RO", Payload.rejectreason, Payload.Id,"ecod_smc_monitor").subscribe({
        next: (res: any) => {
          this.getsmcMontoringdetails();
          this.snackBar.open('Data Saved Successfully!', 'Close', { duration: 3000 });
        },
      ...
      this.coreservices.CommonApprovalforallroles("FO", "BO", Payload.Id, "ecod_smc_monitor").subscribe({
        next: (res: any) => {
          this.getsmcMontoringdetails();
          this.snackBar.open('Data Saved Successfully!', 'Close', { duration: 3000 });
        },
      ...
      this.coreservices.CommonApprovalforallroles("BO", "RO", Payload.Id, "ecod_smc_monitor").subscribe({
        next: (res: any) => {
          this.getsmcMontoringdetails();
          this.snackBar.open('Data Saved Successfully!', 'Close', { duration: 3000 });
        },
      ...
      this.coreservices.commonApprovedByRangeo(Payload.Id,"ecod_smc_monitor").subscribe({
        next: (res: any) => {
          this.getsmcMontoringdetails();
          this.snackBar.open('Data Saved Successfully!', 'Close', { duration: 3000 });
        },
