...
@Component({
  ...
  styleUrls: ['./maintence.component.css']
})
...
columns: GridColumn[] = [
  ...
  { field: 'activity_in_hec', header: 'Activity in Hectare' },
  ...
  },
 {
  ...
  // ================= LOAD DATA =================
getmaintencedata() {
  this.coreservices.getmaintencedatalist(this.userid).subscribe({
     next: (res: any) => {
      ...
      sessionStorage.setItem("Plant Maintence", JSON.stringify(rawData));
 ...
 openLocationOnMap(row: any) {
  ...
  openImage(id: number, imageName: string) {
    ...
    this.popupImageUrl = ''; // Set loading state

    this.coreservices.getplantationmaintenceImages(id, imageName, 'plantation_maintenence').subscribe({
      next: (res: any) => {
  ...
  downloadKmlByRow(row: any) {
    ...
    let Payload = {
      ...
      plantation_id:payload.plantation_id || row.plantation_id,
    ...
    if (Payload.comments != undefined && Payload.status != "NO" && Payload.status != "YES") {
      this.coreservices.Insertplantationmaintencedetails(Payload).subscribe({
        next: (res: any) => {
          this.getmaintencedata();
      ...
      this.coreservices.Insertcommonrejectdetails("RO", Payload.rejectreason, Payload.Id,"plantation_maintanance").subscribe({
        next: (res: any) => {
          this.getmaintencedata();
          this.snackBar.open('Data Saved Successfully!', 'Close', { duration: 3000 });
        },
      ...
      this.coreservices.CommonApprovalforallroles("FO", "BO", Payload.Id, "plantation_maintanance").subscribe({
        next: (res: any) => {
          this.getmaintencedata();
          this.snackBar.open('Data Saved Successfully!', 'Close', { duration: 3000 });
        },
      ...
      this.coreservices.CommonApprovalforallroles("BO", "RO", Payload.Id, "plantation_maintanance").subscribe({
        next: (res: any) => {
          this.getmaintencedata();
          this.snackBar.open('Data Saved Successfully!', 'Close', { duration: 3000 });
        },
      ...
      this.coreservices.commonApprovedByRangeo(Payload.Id,"plantation_maintanance").subscribe({
        next: (res: any) => {
          this.getmaintencedata();
          this.snackBar.open('Data Saved Successfully!', 'Close', { duration: 3000 });
        },
