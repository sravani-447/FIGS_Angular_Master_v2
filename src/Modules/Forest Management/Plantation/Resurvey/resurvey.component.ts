...
@Component({
  selector: 'app-resurvey.component',
  templateUrl: './resurvey.component.html',
  styleUrls: ['./resurvey.component.css']
})
export class resurveyComponent implements OnInit {
...
columns: GridColumn[] = [
  // Action buttons
 {
  ...
  { field: 'type_of_survey', header: 'Survey_Type' },
  { field: 'area_hect', header: 'Area_Hect' },
  { field: 'length_km', header: 'Length_Km' },
  ...
  // ================= LOAD DATA =================
getresurveydata() {
  this.coreservices.getResurveydatalist(this.userid).subscribe({
     next: (res: any) => {
 ...
 openLocationOnMap(row: any) {
  ...
  openImage(id: string, imageName: string) {
    ...
    this.coreservices.getresurveyImages(id, imageName).subscribe({
      next: (res: any) => {
...
downloadKmlByRow(row: any) {
    ...
    const surveyType = row.type_of_survey || row.survey_type;
    ...
    try {
      ...
      a.download = `plantation_resurvey_${row.plantation_id || 'survey'}.kml`;
      a.href = url;
    ...
    if (Payload.comments != undefined && Payload.status != "NO" && Payload.status != "YES") {
      this.coreservices.Insertplantationresurveydetails(Payload).subscribe({
        next: (res: any) => {
          this.getresurveydata();
      ...
      this.coreservices.Insertcommonrejectdetails("RO", Payload.rejectreason, Payload.Id,"plantation_resurvey").subscribe({
        next: (res: any) => {
          this.getresurveydata();
          this.snackBar.open('Data Saved Successfully!', 'Close', { duration: 3000 });
        },
      ...
      this.coreservices.CommonApprovalforallroles("FO", "BO", Payload.Id, "plantation_resurvey").subscribe({
        next: (res: any) => {
          this.getresurveydata();
          this.snackBar.open('Data Saved Successfully!', 'Close', { duration: 3000 });
        },
      ...
      this.coreservices.CommonApprovalforallroles("BO", "RO", Payload.Id, "plantation_resurvey").subscribe({
        next: (res: any) => {
          this.getresurveydata();
          this.snackBar.open('Data Saved Successfully!', 'Close', { duration: 3000 });
        },
      ...
      this.coreservices.commonApprovedByRangeo(Payload.Id,"plantation_resurvey").subscribe({
        next: (res: any) => {
          this.getresurveydata();
          this.snackBar.open('Data Saved Successfully!', 'Close', { duration: 3000 });
        },
