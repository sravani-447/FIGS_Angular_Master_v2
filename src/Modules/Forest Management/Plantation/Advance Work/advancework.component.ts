...
export class advanceworkComponent implements OnInit {
...
columns: GridColumn[] = [
  // Action buttons
 {
  ...
  // Work details
  { field: 'no_of_pit_dug', header: 'No of Pit Dug' },
  { field: 'advance_work_completed_hect', header: 'Advance Work Completed(hect)' },
  { field: 'advance_work_completed_km', header: 'Advance Work Completed(Km)' },
  { field: 'total_cost', header: 'Total cost' },
  { field: 'mand_male', header: 'No. of mandays male' },
  { field: 'mand_female', header: 'No. of mandays female' },
  ...
  // ================= LOAD DATA =================
getadvanceworkdata() {
  this.coreservices.getadvancedatalist(this.userid).subscribe({
    next: (res: any) => {
 ...
 openLocationOnMap(row: any) {
  ...
  openImage(id: string, imageName: string) {
    ...
    this.coreservices.getadvanceworkImages(id, imageName).subscribe({
      next: (res: any) => {
  ...
  onGridAction(event: { action: string; row: any }) {
    ...
    let Payload = {
      ...
      type_of_survey: payload.type_of_survey,
      no_of_pit_dug: payload.no_of_pit_dug,
      advance_work_completed_hect:payload.advance_work_completed_hect,
      advance_work_completed_km:payload.advance_work_completed_km,
      total_cost:payload.total_cost,
      no_mandays:payload.no_mandays,
    ...
    if (Payload.comments != undefined && Payload.status != "NO" && Payload.status != "YES") {
      this.coreservices.Insertplantationadvanceworkedetails(Payload).subscribe({
        next: (res: any) => {
          this.getadvanceworkdata();
      ...
      this.coreservices.Insertcommonrejectdetails("RO", Payload.rejectreason, Payload.Id,"plantation_advance").subscribe({
        next: (res: any) => {
          this.getadvanceworkdata();
          this.snackBar.open('Data Saved Successfully!', 'Close', { duration: 3000 });
        },
      ...
      this.coreservices.CommonApprovalforallroles("FO", "BO", Payload.Id, "plantation_advance").subscribe({
        next: (res: any) => {
          this.getadvanceworkdata();
          this.snackBar.open('Data Saved Successfully!', 'Close', { duration: 3000 });
        },
      ...
      this.coreservices.CommonApprovalforallroles("BO", "RO", Payload.Id, "plantation_advance").subscribe({
        next: (res: any) => {
          this.getadvanceworkdata();
          this.snackBar.open('Data Saved Successfully!', 'Close', { duration: 3000 });
        },
      ...
      this.coreservices.commonApprovedByRangeo(Payload.Id,"plantation_advance").subscribe({
        next: (res: any) => {
          this.getadvanceworkdata();
          this.snackBar.open('Data Saved Successfully!', 'Close', { duration: 3000 });
        },
