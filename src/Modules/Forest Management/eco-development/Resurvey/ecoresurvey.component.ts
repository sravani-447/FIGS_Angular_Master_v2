...
@Component({
  ...
  templateUrl: './ecoresurvey.component.html',
  styleUrls: ['./ecoresurvey.component.css']
})
export class ecoresurveyComponent implements OnInit {
  ...
  },
{
  ...
  // ================= LOAD DATA =================
  getresurveydata() {
    this.coreservices.getecoresurveydatalist(this.userid).subscribe({
      next: (res: any) => {
        ...
        console.log(rawData);
        sessionStorage.setItem("Eco Resurvey Details", rawData)
        console.log("Final Parsed Data:", rawData);
 ...
 openLocationOnMap(row: any) {
  ...
  openImage(id: string, imageName: string) {
    ...
    this.coreservices.geteceoresurveyImages(id, imageName).subscribe({
      next: (res: any) => {
...
downloadKmlByRow(row: any) {
    ...
    try {
      ...
      a.download = `eco_plantation_resurvey_${row.plantation_id || 'survey'}.kml`;
      a.href = url;
    ...
    if (Payload.comments != undefined && Payload.status != "NO" && Payload.status != "YES") {
      this.coreservices.InsertEcoplantationresurveydetails(Payload).subscribe({
        next: (res: any) => {
          this.getresurveydata();
      ...
      this.coreservices.Insertcommonrejectdetails("RO", Payload.rejectreason, Payload.Id,"ecod_plan_resurvey").subscribe({
        next: (res: any) => {
          this.getresurveydata();
          this.snackBar.open('Data Saved Successfully!', 'Close', { duration: 3000 });
        },
      ...
      this.coreservices.CommonApprovalforallroles("FO", "BO", Payload.Id, "ecod_plan_resurvey").subscribe({
        next: (res: any) => {
          this.getresurveydata();
          this.snackBar.open('Data Saved Successfully!', 'Close', { duration: 3000 });
        },
      ...
      this.coreservices.CommonApprovalforallroles("BO", "RO", Payload.Id, "ecod_plan_resurvey").subscribe({
        next: (res: any) => {
          this.getresurveydata();
          this.snackBar.open('Data Saved Successfully!', 'Close', { duration: 3000 });
        },
      ...
      this.coreservices.commonApprovedByRangeo(Payload.Id,"ecod_plan_resurvey").subscribe({
        next: (res: any) => {
          this.getresurveydata();
          this.snackBar.open('Data Saved Successfully!', 'Close', { duration: 3000 });
        },
