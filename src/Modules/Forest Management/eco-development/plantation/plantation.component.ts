...
export class ecoplantationComponent implements OnInit {
...
showSpeciesPopup = false;
...

  {
    ...
    type: 'actions',
    actions: [
      { icon: 'fa fa-bars', action: 'species_details' }
    ]
  },
  ...
  { field: 'no_of_signboard', header: 'No. of Signboards' },
  { field: 'plantation_completed_km', header: 'Plantation Completed KM' },
  { field: 'plantation_completed_hect', header: 'Plantation Completed Hectare' },
      ...
      actions: [
        {
          ...
          visible: (row: any) => !!row.image1_name
        }
      ]
    },
      ...
      actions: [
        {
          ...
          visible: (row: any) => !!row.image2_name
        }
      ]
    },
      ...
      actions: [
        {
          ...
          visible: (row: any) => row.lat && row.lng
        }
      ]
    }
  ...
  // ================= LOAD DATA =================
  getecoPlantation() {
    this.coreservices.getecoplantationdatalist(this.userid).subscribe({
       next: (res: any) => {
...
openImage(id: string, imageName: string) {
    ...
    this.coreservices.geteceoplantationaImages(id, imageName, 'ecod_plan_plantation').subscribe({
      next: (res: any) => {
  ...
  onGridAction(event: { action: string; row: any }) {
    ...
    switch (event.action) {
      ...
      this.getFeatureInfo(row);
      break;
    }
  }

getFeatureInfo(row: any): void {
  this.speciesDetails = [];

  if (!row?.species_seedling_details) {
    return;
  }

  let speciesD = row.species_seedling_details;

  this.speciesDetails = speciesD
    .replace(/\[|\]/g, '')
    .split('LstChild')
    .filter((x: string) => x.trim() !== '')
    .map((x: string) => {
      const nameMatch = x.match(/name_of_species\s*=\s*([^,]+)/);
      const seedMatch = x.match(/no_seedling\s*=\s*(\d+)/);

      return {
        name_of_species: nameMatch ? nameMatch[1].trim() : '',
        no_seedling: seedMatch ? parseInt(seedMatch[1], 10) : 0
      };
    });
...
closeSpeciesPopup(): void {
  ...
  handleStatusUpdate(payload: any) {
    ...
    let Payload = {
      ...
      Livefencingwithnameplate: payload.no_of_lines,
      live_fencing_with_nameplate: payload.live_fencing_with_nameplate,
      Noofsignboard: payload.no_of_signboard,
      Plantationcompletedinkm: payload.plantation_completed_km,
      Line:payload.no_of_lines,
    ...
    if (Payload.comments != undefined && Payload.status != "NO" && Payload.status != "YES") {
      this.coreservices.InsertEcoplantationplantationdetails(Payload).subscribe({
        next: (res: any) => {
          this.getecoPlantation();
      ...
      this.coreservices.Insertcommonrejectdetails("RO", Payload.rejectreason, Payload.Id, "ecod_plan_plantation").subscribe({
        next: (res: any) => {
          this.getecoPlantation();
          this.snackBar.open('Data Saved Successfully!', 'Close', { duration: 3000 });
        },
      ...
      this.coreservices.CommonApprovalforallroles("FO", "BO", Payload.Id, "ecod_plan_plantation").subscribe({
        next: (res: any) => {
          this.getecoPlantation();
          this.snackBar.open('Data Saved Successfully!', 'Close', { duration: 3000 });
        },
      ...
      this.coreservices.CommonApprovalforallroles("BO", "RO", Payload.Id, "ecod_plan_plantation").subscribe({
        next: (res: any) => {
          this.getecoPlantation();
          this.snackBar.open('Data Saved Successfully!', 'Close', { duration: 3000 });
        },
      ...
      this.coreservices.commonApprovedByRangeo(Payload.Id, "ecod_plan_plantation").subscribe({
        next: (res: any) => {
          this.getecoPlantation();
          this.snackBar.open('Data Saved Successfully!', 'Close', { duration: 3000 });
        },
