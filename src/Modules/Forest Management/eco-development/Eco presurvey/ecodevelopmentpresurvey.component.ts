...
@Component({
  selector: 'app-ecodevelopmentpresurvey.component',
  templateUrl: './ecodevelopmentpresurvey.component.html',
  styleUrls: ['./ecodevelopmentpresurvey.component.css']
})
export class ecodevelopmentpresurveyComponent implements OnInit {
 ...
 {field:'survey_type',header:'Type Survey'},
    { field: 'id', header: 'Id' },
    { field: 'plantation_id', header: 'Plantation_Id' },
    { field: 'beat_name', header: 'Beat Name' },
    { field: 'area_hect', header: 'Area(Hect)' },
    { field: 'length_km', header: 'Length(Km)' },
    { field: 'locatity_name', header: 'Locatity_Name' },
    ...
    // Map Action
    {
      ...
      header: 'Zoom',
      type: 'actions',
      actions: [{ icon: 'fa fa-map-marker', action: 'map', tooltip: 'Zoom To Map' }]
    },
...

 {
  ...
  // ================= LOAD DATA =================
  getecodevelopmentdata() {
    this.coreservices.getecodevelopmentplantationpresurveydatalist(this.userid).subscribe({
       next: (res: any) => {
      ...
      sessionStorage.setItem("Eco development Pre survey", JSON.stringify(rawData));
 ...
 openLocationOnMap(row: any) {
  ...
  openImage(id: string, imageName: string) {
    ...
    this.coreservices.geteceoplantationpresurveyImages(id, imageName).subscribe({
      next: (res: any) => {
  ...
  downloadKmlByRow(row: any) {
    ...
    try {
      ...
      a.download = `eco_plantation_presurvey_${row.plantation_id || 'survey'}.kml`;
      a.href = url;
    ...
    if (Payload.comments != undefined && Payload.status != "NO" && Payload.status != "YES") {
      this.coreservices.InsertEcoplantationpresurveydetails(Payload).subscribe({
        next: (res: any) => {
          this.getecodevelopmentdata();
      ...
      this.coreservices.Insertcommonrejectdetails("RO", Payload.rejectreason, Payload.Id,"ecod_plan_presurvey").subscribe({
        next: (res: any) => {
          this.getecodevelopmentdata();
          this.snackBar.open('Data Saved Successfully!', 'Close', { duration: 3000 });
        },
      ...
      this.coreservices.CommonApprovalforallroles("FO", "BO", Payload.Id, "ecod_plan_presurvey").subscribe({
        next: (res: any) => {
          this.getecodevelopmentdata();
          this.snackBar.open('Data Saved Successfully!', 'Close', { duration: 3000 });
        },
      ...
      this.coreservices.CommonApprovalforallroles("BO", "RO", Payload.Id, "ecod_plan_presurvey").subscribe({
        next: (res: any) => {
          this.getecodevelopmentdata();
          this.snackBar.open('Data Saved Successfully!', 'Close', { duration: 3000 });
        },
      ...
      this.coreservices.commonApprovedByRangeo(Payload.Id,"ecod_plan_presurvey").subscribe({
        next: (res: any) => {
          this.getecodevelopmentdata();
          this.snackBar.open('Data Saved Successfully!', 'Close', { duration: 3000 });
        },
