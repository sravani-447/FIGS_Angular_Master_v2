...
export class ecodevelopmentsitemasterComponent implements OnInit {
...
columns: GridColumn[] = [
  ...
  { field: 'id', header: 'Id' },
  { field: 'type_of_survey', header: 'Type_of_Survey' },
  { field: 'plantation_id', header: 'Plantation_Id' },
  { field: 'selected_mode_id', header: 'Type_of_Plantation' },
  { field: 'type_of_scheme', header: 'Type_of_Scheme' },
  { field: 'scheme_lst', header: 'Scheme_Lst' },
  { field: 'selected_mode', header: 'Mode' },
  { field: 'selected_mode', header: 'Mode Type Name' },
  ...
  getecositemaster() {
    this.coreservices.getEcodevelopmentSiteMasterDatalist(this.userid).subscribe({
       next: (res: any) => {
 ...
 openLocationOnMap(row: any) {
  ...
  openImage(id: string, imageName: string) {
    ...
    this.coreservices.getEcoSiteMasterImages(id, imageName).subscribe({
      next: (res: any) => {
  ...
  onGridAction(event: { action: string; row: any }) {
    ...
    if (Payload.comments != undefined && Payload.status != "NO" && Payload.status != "YES") {
      this.coreservices.InsertEcoplantationsitemasterdetails(Payload).subscribe({
        next: (res: any) => {
          this.getecositemaster();
      ...
      this.coreservices.Insertcommonrejectdetails("RO", Payload.rejectreason, Payload.Id,"ecod_plan_site_master").subscribe({
        next: (res: any) => {
          this.getecositemaster();
          this.snackBar.open('Data Saved Successfully!', 'Close', { duration: 3000 });
        },
      ...
      this.coreservices.CommonApprovalforallroles("FO", "BO", Payload.Id, "ecod_plan_site_master").subscribe({
        next: (res: any) => {
          this.getecositemaster();
          this.snackBar.open('Data Saved Successfully!', 'Close', { duration: 3000 });
        },
      ...
      this.coreservices.CommonApprovalforallroles("BO", "RO", Payload.Id, "ecod_plan_site_master").subscribe({
        next: (res: any) => {
          this.getecositemaster();
          this.snackBar.open('Data Saved Successfully!', 'Close', { duration: 3000 });
        },
      ...
      this.coreservices.commonApprovedByRangeo(Payload.Id,"ecod_plan_site_master").subscribe({
        next: (res: any) => {
          this.getecositemaster();
          this.snackBar.open('Data Saved Successfully!', 'Close', { duration: 3000 });
        },
